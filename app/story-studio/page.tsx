...
export default function StoryStudioPage() {
  ...
  const [verifyResults, setVerifyResults] = useState<Array<{ id: string; text: string; currentCategory: string; suggestedCategory: string; reason: string; accepted: boolean }>>([]);
  ...
  useEffect(() => {
    ...
    setTotalEntries(a.entries.length);
    setLoaded(true);
  }, []);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--va-bg)", color: "var(--va-text)", padding: "2rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
        ...
        {hasGeminiKey() && totalEntries > 0 && (
          <button onClick={async () => {
            ...
            const a = loadArchive();
            const entries = a.entries.map(e => ({ id: e.id, text: e.text, category: e.category }));
            const ALL_CATS = Object.keys(CATEGORY_LABELS);
            const results = await geminiVerifyCategories(entries, ALL_CATS);
            // Default all to accepted — user clicks to reject
            setVerifyResults(results.map(r => ({ ...r, accepted: true })));
        ...
        <div style={{ background: "var(--va-surface)", border: "1px solid #7c3aed", borderRadius: "0.75rem", padding: "1.25rem", marginBottom: "1.5rem" }}>
            ...
            <div>
              ...
              <p style={{ fontSize: "0.75rem", color: "var(--va-text-muted)" }}>
                {verifyResults.length === 0
                  ? "All entries are in the correct categories ✓"
              ...
              {verifyResults.length > 0 && (
                <>
                  <button onClick={() => {
                    ...
                    let updated = { ...a, entries: [...a.entries] };
                    toMove.forEach(r => {
                    ...
                    const counts: Partial<Record<StoryCategory, number>> = {};
                    for (const entry of fresh.entries) counts[entry.category] = (counts[entry.category] ?? 0) + 1;
                    setEntryCounts(counts);
          ...
          {verifyResults.map((r, i) => (
            <div key={r.id} style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start", background: "var(--va-bg)", border: `1px solid ${r.accepted ? "#7c3aed" : "var(--va-border)"}`, borderRadius: "0.5rem", padding: "0.75rem", marginBottom: "0.5rem", opacity: r.accepted ? 1 : 0.5 }}>
              ...
              <button
                onClick={() => setVerifyResults(prev => prev.map((item, idx) => idx === i ? { ...item, accepted: !item.accepted } : item))}
      ...
      <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search categories..."
        style={{ background: "var(--va-surface)", border: "1px solid var(--va-border)", borderRadius: "0.5rem", padding: "0.75rem 1rem", outline: "none", color: "var(--va-text)", fontSize: "0.875rem", width: "100%", maxWidth: "28rem", display: "block", marginBottom: "2rem", boxSizing: "border-box" }} />

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "0.75rem" }}>
        {filteredCategories.map((category) => {
          const count = entryCounts[category] ?? 0;
          const priority = getPriorityLevel(archive, `story-${category}`);
          const priorityDot = priority === "red" ? "🔴" : priority === "blue" ? "🔵" : null;
          return (
            <Link key={category} href={`/story-studio/${category}`}
              style={{ background: "var(--va-surface)", border: `1px solid ${priority === "red" ? "#ef4444" : priority === "blue" ? "#3b82f6" : "var(--va-border)"}`, borderRadius: "0.75rem", padding: "1rem", textDecoration: "none", display: "block", opacity: count > 0 ? 1 : 0.7, transition: "border-color 0.2s, opacity 0.2s" }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--va-accent)"; e.currentTarget.style.opacity = "1"; }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = priority === "red" ? "#ef4444" : priority === "blue" ? "#3b82f6" : "var(--va-border)"; e.currentTarget.style.opacity = count > 0 ? "1" : "0.7"; }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem" }}>
                ...
                <div style={{ display: "flex", gap: "0.25rem", alignItems: "center" }}>
                  ...
                  {count > 0 && (
                    <span style={{ fontSize: "0.7rem", background: "rgba(59,130,246,0.2)", color: "var(--va-accent)", padding: "0.125rem 0.5rem", borderRadius: "9999px" }}>
                      {count}
                    </span>
                  )}
                </div>
