...
export default function CategoryPage() {
  ...
  // Entries filtered by subcategory prefix and search
  const entries = allEntries.filter((e) => {
    const matchesSub = filterSub === "all" || e.text.startsWith(`[${filterSub}] `);
  ...
  // Get subcategory tag from entry text
  ...
  // Count entries per subcategory
  const subCounts: Record<string, number> = {};
  for (const entry of allEntries) {
    const tag = getSubTag(entry.text);
    if (tag) subCounts[tag] = (subCounts[tag] ?? 0) + 1;
    else subCounts["__untagged__"] = (subCounts["__untagged__"] ?? 0) + 1;
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--va-bg)", color: "var(--va-text)", display: "flex", flexDirection: "column" }}>
      ...
      {/* Header */}
      <div style={{ padding: "1.5rem 2rem", borderBottom: "1px solid var(--va-border)" }}>
        <Link href="/dashboard" style={{ ...S.muted, fontSize: "0.875rem", display: "inline-block", marginBottom: "0.25rem" }}>← Home</Link>
          ...
          <aside style={{ width: "14rem", borderRight: "1px solid var(--va-border)", background: "var(--va-surface)", padding: "1rem", flexShrink: 0 }}>
            ...
            <nav style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
              ...
              {subcategories.map((sub) => (
                ...
                >
                  <span>{sub}</span>
                  {(subCounts[sub] ?? 0) > 0 && <span style={{ fontSize: "0.7rem", color: "var(--va-accent)", opacity: 0.8 }}>{subCounts[sub]}</span>}
                </button>
              ))}
            </nav>
          </aside>
        ...
        <main style={{ flex: 1, padding: "1.5rem 2rem" }}>
          ...
          {entries.length === 0 ? (
            <div style={{ textAlign: "center", paddingTop: "4rem" }}>
              ...
              <p style={S.muted}>
                {allEntries.length === 0
                  ? "No entries yet. Add above or paste into Inbox."
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              {entries.map((entry) => {
                const subTag = getSubTag(entry.text);
                  ...
                  <div key={entry.id} style={{ ...S.surface, padding: "1rem" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                        {subTag && (
                          <span style={{ fontSize: "0.7rem", background: "var(--va-border)", color: "var(--va-accent)", padding: "0.125rem 0.5rem", borderRadius: "9999px" }}>
                            {subTag}
                          </span>
                        )}
